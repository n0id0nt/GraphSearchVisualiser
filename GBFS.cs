...
namespace Search
{
    class GBFS : InformedSearch
    {
        ...
        public GBFS(Node startingNode, Enviroment enviroment) : base(startingNode, enviroment)
        {
        ...
        public override string RunSearch()
        {
            ...
            while (fronteir.Count != 0)
            {
                ...
                int minScore = MovePortential(fronteir[0].X, fronteir[0].Y);

                for (int i = 1; i < fronteir.Count; i++)
                {
                    int curScore = MovePortential(fronteir[i].X, fronteir[i].Y);
